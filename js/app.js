...
class LuminaApp {
  ...
  bindEvents() {
    ...
    this.dom.zoneTouchPrev.addEventListener("click", (e) => {
      ...
      if (this.ruler && this.ruler.enabled) {
        this.ruler.stepLine(-1); // Červená zóna vlevo -> krok zpět o slovo/řádek
      } else {
        this.prevPage();
      }
    });
    ...
    this.dom.zoneTouchNext.addEventListener("click", (e) => {
      ...
      if (this.ruler && this.ruler.enabled) {
        this.ruler.stepLine(1); // Zelená zóna vpravo -> krok vpřed o slovo/řádek
      } else {
        this.nextPage();
      }
    });
    ...
    this.dom.pagedViewport.addEventListener("touchstart", (e) => {
      // Apple Pencil nebo aktivní interakce s pravítkem: nesmí zahajovat swipe gesta otáčení stránek
      if (this.ruler?.isInteracting() || (this.ruler?.enabled && this.ruler?.isPenEvent(e))) {
        isSwiping = false;
        return;
      }
    ...
    this.dom.pagedViewport.addEventListener("touchend", (e) => {
      // Apple Pencil nebo aktivní interakce s pravítkem / dokončené podržení: ignorovat ukončení dotyku pro gesta listování i tap-to-step
      if (this.ruler?.isInteracting() || this.ruler?.isHoldTriggered || (this.ruler?.enabled && this.ruler?.isPenEvent(e))) {
        if (this.ruler) this.ruler.isHoldTriggered = false;
        isSwiping = false;
        return;
      }
      if (!isSwiping) return;
      ...
      // 1. Horizontální přejetí (swipe): deltaX musí převyšovat deltaY a překročit prahovou vzdálenost
      if (Math.abs(deltaX) > Math.abs(deltaY) * 1.2 && Math.abs(deltaX) > 40 && deltaTime < 750) {
        lastSwipeTime = Date.now();
        if (deltaX < 0) {
          // Přejetí doleva -> Další strana (pouze 1)
          this.nextPage();
        } else {
          // Přejetí doprava -> Předchozí strana (pouze 1)
          this.prevPage();
        }
        return;
      }

      // 2. Klepnutí na obrazovku (Tap) pro ovládání pravítka na dotykovém zařízení podle schématu
      if (deltaTime < 450 && Math.abs(deltaX) < 18 && Math.abs(deltaY) < 18) {
        if (this.ruler && this.ruler.enabled) {
          if (this.ruler.isInteracting() || this.ruler.isHoldTriggered || Date.now() < this.ruler.dragCooldownEndTime) {
    ...
    this.dom.pagedViewport.addEventListener("click", (e) => {
      ...
      // Apple Pencil nebo probíhající interakce/cooldown nesmí vyvolávat krokovací tap
      if (this.ruler.isInteracting() || this.ruler.isHoldTriggered || Date.now() < this.ruler.dragCooldownEndTime || this.ruler.isPenEvent(e)) {
        this.ruler.isHoldTriggered = false;
        return;
      }
      if (Date.now() - lastSwipeTime < 500 || Date.now() - lastTapTime < 500) {
  ...
  getRulerTapDirection(clientX, clientY) {
    if (!this.dom.pagedViewport) return 1;
    const rect = this.dom.pagedViewport.getBoundingClientRect();
